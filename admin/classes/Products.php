...
class Products
{
	...
	public function getProducts(){
		$q = $this->con->query("SELECT p.product_id, p.product_title, p.product_price,p.product_qty, p.product_desc, p.product_image, p.product_keywords, c.cat_title, c.cat_id, b.discount_id, b.discount_title FROM products p JOIN categories c ON c.cat_id = p.product_cat JOIN brands b ON b.discount_id = p.product_discount");
	...
	public function addProduct($product_name,
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/Food-Project-master/product_images/".$uniqueImageName)) {
					
					$q = $this->con->query("INSERT INTO `products`(`product_cat`, `product_discount`, `product_title`, `product_qty`, `product_price`, `product_desc`, `product_image`, `product_keywords`) VALUES ('$category_id', '$discount_id', '$product_name', '$product_qty', '$product_price', '$product_desc', '$uniqueImageName', '$product_keywords')");
	...
	public function addBrand($name){
		$q = $this->con->query("SELECT * FROM brands WHERE discount_title = '$name' LIMIT 1");
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Discount already exists'];
		}else{
			$q = $this->con->query("INSERT INTO brands (discount_title) VALUES ('$name')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'New Discount added Successfully'];
			}else{
				return ['status'=> 303, 'message'=> 'Failed to run query'];
			}
		}
	...
	public function deleteBrand($bid = null){
		if ($bid != null) {
			$q = $this->con->query("DELETE FROM brands WHERE discount_id = '$bid'");
			if ($q) {
				return ['status'=> 202, 'message'=> 'Discount removed'];
			}else{
				return ['status'=> 202, 'message'=> 'Failed to run query'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'Invalid Discount id'];
		}
	...
	public function updateBrand($post = null){
		...
		if (!empty($discount_id) && !empty($e_discount_title)) {
			$q = $this->con->query("UPDATE brands SET discount_title = '$e_discount_title' WHERE discount_id = '$discount_id'");
			if ($q) {
...
if (isset($_POST['add_brand'])) {
	if (isset($_SESSION['admin_id'])) {
		...
		if (!empty($discount_title)) {
			$p = new Products();
			echo json_encode($p->addBrand($discount_title));
		}else{
			echo json_encode(['status'=> 303, 'message'=> 'Empty fields']);
		}
	}else{
